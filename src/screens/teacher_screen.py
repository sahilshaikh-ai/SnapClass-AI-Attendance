...
def teacher_dashboard():
    ...
    with c2: 
        ...
        if st.button("Logout", type="secondary", key="loginbackbtn", shortcut="backspace"):
...
def teacher_tab_take_attendance():
    ...
    if not subjects: 
        st.warning("You haven't created any subjects yet! Please create a one to begin!")
        return
    ...
    with c1: 
        if st.button("Clear all photos",width="stretch",type="tertiary",icon=":material/delete:",disabled=not has_photo):
            st.session_state.attendance_images = []
            st.rerun()

    with c2:
        if st.button("Run Face Analysis", width="stretch", type="primary",icon=":material/analytics:", disabled=not has_photo):
